...
export class LogViewerProvider extends BaseLogProvider {
  ...
  public async appendNewEntries(
    ...
    // Check custom alert patterns against raw lines first (independent of log format parsing)
    this.checkAlertPatterns(path.basename(filePath), newLines);
  ...
  private async readAndCheckAlerts(filePath: string, from: number, to: number): Promise<void> {
  ...
  private checkAlertPatterns(fileName: string, lines: string[]): void {
    const workspaceUri = vscode.workspace.workspaceFolders?.[0]?.uri || null;
    const config = vscode.workspace.getConfiguration(
      "magentoLogViewer",
      workspaceUri,
  ...
  public groupLogEntries(lines: string[], filePath: string): LogItem[] {
    const groupedByType = new Map<
      string,
      { message: string; line: string; lineNumber: number }[]
    >();
      ...
      .map(([level, entries]) => {
        if (this.groupByMessage) {
          ...
          const topicGroups = Array.from(groupedByTopic.entries())
            .map(([topic, topicEntries]) => {
              ...
              const logEntries = topicEntries
                .map((entry) => {
                  const lineNumber = (entry.lineNumber + 1)
                    .toString()
                    .padStart(2, "0");
          ...
          const filteredEntries = entries.filter(
            (entry) =>
              this.matchesSearchTerm(entry.message) ||
              this.matchesSearchTerm(entry.line),
          );
  ...
  private groupByTopics(
    entries: { message: string; line: string; lineNumber: number }[],
  ): Map<string, { message: string; line: string; lineNumber: number }[]> {
